...
def main():
    index = json.loads((ROOT / "artifacts/fly64/latest-replay.index.json").read_text())
    if index.get("schema") != 3:
        raise ValueError("Record a new first-person run before testing causality")
    sequences = []
    for name in index["chunks"][:3]:
        with np.load(ROOT / "artifacts" / "fly64" / name) as chunk:
    ...
    (ROOT / "artifacts/fly64/causality.json").write_text(json.dumps(report, indent=2))
