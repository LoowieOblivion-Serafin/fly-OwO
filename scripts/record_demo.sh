...
MP4="${2:-$PROJECT_DIR/artifacts/fly64/fly64-demo-$(date +%Y%m%d-%H%M%S).mp4}"
mkdir -p "$PROJECT_DIR/runtime" "$PROJECT_DIR/artifacts/fly64"
xcrun swiftc -parse-as-library "$PROJECT_DIR/scripts/record_windows.swift" -o "$PROJECT_DIR/runtime/record-windows"
export FLY64_VERIFY_VISION="${FLY64_VERIFY_VISION:-$PROJECT_DIR/artifacts/fly64/vision-isolation.json}"
