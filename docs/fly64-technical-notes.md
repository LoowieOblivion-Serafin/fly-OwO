# Fly64 technical notes

For the original run/build instructions, see [the Fly64 README](fly64-readme.md).
These upstream notes describe `fly64/` and Mario, including first-person vision.
For the SRB2 adaptation and its distinct bridge/replay format, see the
[Fly Sonic notes](technical-notes.md).
...
.venv/bin/pytest -q
PYTHONPATH=. .venv/bin/python scripts/validate_fly64_causality.py
.venv/bin/python -m fly64.replay artifacts/fly64/latest-replay.index.json
```
...
In this fork, local results are written to `artifacts/fly64/causality.json`,
`artifacts/fly64/soak-report.json`, replay chunks, logs and recordings when those
runs complete. The soak checks execution stability, not biological validity.
This namespace keeps Mario records separate from Sonic's `artifacts/latest-replay.*`.
The native Fly64 game and recording launcher target macOS; portable Python tests
on Windows do not validate the Mario build or its camera/rendering audit.
