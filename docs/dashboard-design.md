...
This document describes the upstream **Fly64/Mario** dashboard in `web/index.html`,
`web/dashboard.css`, and `web/dashboard.js`, served by `fly64.main`. The retained
Fly Sonic dashboard lives in `web/sonic/index.html`, is served by `flysonic.main`,
and uses its original packet format and 64×48 SRB2 framebuffer input. See the
[Sonic technical notes](technical-notes.md) for that separate interface.
